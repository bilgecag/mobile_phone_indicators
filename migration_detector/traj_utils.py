...
def change_overlap_segment(x, filter_segment_col, k, d):
    ...
    for location in loc_list:
        ...
        if len(current_loc_changed_date) > 0:
            current_loc_changed_date.sort() # edited by David to correct the indentation
...
def plot_traj_common(traj, user_id, start_day, end_day, date_num_long):
    ...
    # Format and calculations remain similar
    ...
    appear_loc.sort()
    date_plot = list(range(start_day, end_day + 1)) # edited by David to format as list.
